...
export function TraceTimeline({ items, embedded = false }: { items: TraceEvent[]; embedded?: boolean }) {
  const [expanded, setExpanded] = useState<number | null>(items[items.length - 1]?.id ?? null);
  return <div className={embedded ? "trace-view" : "workspace-right"}><div className="trace-header"><div className="trace-title-row"><span className="trace-title">智能体轨迹</span><span className="trace-count">第 {items.length} 步</span></div><div className="progress-bar"><span style={{ width: `${Math.min(100, Math.max(4, items.length * 15))}%` }} /></div></div><div className="trace-list">{items.map((item) => { const isOpen = expanded === item.id; return <div className={`trace-item ${item.kind}`} key={item.id}><div className="trace-node"><TraceIcon item={item} /></div><div className="trace-content"><div className="trace-item-head"><span className="trace-event">{item.label}</span><span className="trace-time">{item.time}</span></div><div className="trace-summary">{item.summary}<button className={`trace-expand ${isOpen ? "open" : ""}`} type="button" title="展开步骤详情" onClick={() => setExpanded(isOpen ? null : item.id)}><ChevronDown /></button></div>{isOpen && item.detail && <div className="trace-detail">{item.detail}</div>}</div></div>; })}</div></div>;
}

...
import { ArrowRight, Bot, Check, FileCode2, FlaskConical, Play, Plus, Settings2, Sparkles, Trash2 } from "lucide-react";
...
export function NewTaskForm() {
  ...
  const [model, setModel] = useState("openai/deepseek-ai/DeepSeek-V4-Flash");
  ...
  const startRun = async () => {
    ...
    try {
      const result = await createRun({ task, model, max_steps: maxSteps, timeout, test_cases: cases, test_case_source: testSource, memory_retrieval: useMemory });
  ...
  return <AppShell breadcrumb="新建任务">
    <main className="page">
      <div className="page-header"><div><h1 className="page-title">给智能体一道题</h1></div><div className="badge blue"><span className="badge-dot" /> Python 工作区</div></div>
      ...
      <div className="form-layout">
        <section className="panel"><div className="panel-header"><span className="panel-title">题目描述</span></div><div className="panel-body"><div className="field"><label className="field-label" htmlFor="problem">你希望智能体解决什么问题？</label><textarea id="problem" className="textarea" value={problem} onChange={(event) => { setProblem(event.target.value); if (testSource === "generated") { setCases([]); setTestSource("manual"); } }} placeholder="粘贴题目、约束条件和样例..." /></div><div className="field"><label className="field-label" htmlFor="notes">补充要求 <span style={{ color: "var(--dim)", fontWeight: 400 }}>（可选）</span></label><input id="notes" className="input" value={notes} onChange={(event) => { setNotes(event.target.value); if (testSource === "generated") { setCases([]); setTestSource("manual"); } }} placeholder="例如：优先使用线性时间复杂度" /></div><div className="field"><div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", gap: 8 }}><div style={{ display: "flex", alignItems: "center", gap: 8 }}><label className="field-label">测试输入与期望输出</label><span className={`badge ${testSource === "generated" ? "purple" : "blue"}`}>{testSource === "generated" ? "AI 生成" : "人工输入"}</span></div><div style={{ display: "flex", gap: 6 }}><button className="secondary-button" type="button" onClick={generateCases} disabled={generating} style={{ height: 28, padding: "0 9px" }}><Sparkles /> {generating ? "生成中..." : "AI 生成"}</button><button className="secondary-button" type="button" onClick={addCase} style={{ height: 28, padding: "0 9px" }}><Plus /> 添加</button></div></div><div style={{ display: "grid", gap: 10 }}>{cases.map((item, index) => <div className="test-case-editor" key={`${item.name ?? "case"}-${index}`}><div className="test-case-editor-head"><span>{item.name || `用例 ${index + 1}`}</span><button className="icon-button" type="button" title="删除测试用例" onClick={() => removeCase(index)}><Trash2 /></button></div><div className="test-case-editor-grid"><div><label className="field-label" htmlFor={`case-input-${index}`}>输入</label><textarea id={`case-input-${index}`} className="case-textarea" placeholder="完整标准输入" value={item.input} onChange={(event) => updateCase(index, "input", event.target.value)} /></div><div><label className="field-label" htmlFor={`case-output-${index}`}>期望输出</label><textarea id={`case-output-${index}`} className="case-textarea" placeholder="完整标准输出" value={item.expected_output} onChange={(event) => updateCase(index, "expected_output", event.target.value)} /></div></div></div>)}</div></div><div className="form-footer"><Link href="/" className="secondary-button">取消</Link><button className="primary-button" type="button" onClick={startRun} disabled={submitting || generating}>{submitting ? "正在创建..." : <><Play /> 运行智能体 <ArrowRight /></>}</button></div></div></section>
        <aside style={{ display: "grid", gap: 16 }}>
        <section className="panel"><div className="panel-header"><span className="panel-title">运行配置</span><Settings2 style={{ width: 14, color: "var(--dim)" }} /></div><div className="panel-body"><div className="field"><label className="field-label" htmlFor="model">模型</label><select id="model" className="select" value={model} onChange={(event) => setModel(event.target.value)}><option value="openai/deepseek-ai/DeepSeek-V4-Flash">DeepSeek V4 Flash（硅基流动）</option></select></div><div className="setting-grid"><div className="field"><label className="field-label" htmlFor="steps">最大步骤</label><input id="steps" className="input" value={maxSteps} onChange={(event) => setMaxSteps(Number(event.target.value))} type="number" min="1" max="100" /></div><div className="field"><label className="field-label" htmlFor="timeout">命令超时（秒）</label><input id="timeout" className="input" value={timeout} onChange={(event) => setTimeoutValue(Number(event.target.value))} type="number" min="1" max="600" /></div></div><div className="field"><label className="field-label">输出语言</label><div className="badge blue" style={{ height: 36, width: "100%", justifyContent: "flex-start", paddingLeft: 11 }}><FileCode2 /> Python 3.11</div></div><label className="memory-toggle"><span><span className="field-label">使用历史记忆</span><span className="field-hint">为当前运行检索并注入已有经验</span></span><input type="checkbox" checked={useMemory} onChange={(event) => setUseMemory(event.target.checked)} /><span className="memory-toggle-track" aria-hidden="true"><span /></span></label></div></section>
          <section className="panel"><div className="panel-header"><span className="panel-title">执行流程</span><Sparkles style={{ width: 14, color: "var(--purple)" }} /></div><div className="panel-body"><div className="pipeline"><div className="pipeline-step done"><div className="pipeline-node"><Check /></div><div className="pipeline-copy"><div className="pipeline-name">准备测试用例</div></div></div><div className="pipeline-step active"><div className="pipeline-node"><Bot /></div><div className="pipeline-copy"><div className="pipeline-name">编写并测试</div></div></div><div className="pipeline-step"><div className="pipeline-node"><FlaskConical /></div><div className="pipeline-copy"><div className="pipeline-name">代码评审</div></div></div></div></div></section>
        </aside>
      </div>
    </main>
  </AppShell>;

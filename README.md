...
项目已加入可选的轨迹经验记忆：验证成功的运行会提炼成功/失败经验，调用硅基流动 Embeddings API 建立向量索引，并保存到本地 SQLite。新任务开始前进行多路语义检索，pytest 失败后进行 Recovery 检索，最多把少量筛选后的经验作为参考注入编程模型。详细设计见 [`doc/MEMORY_V2.md`](doc/MEMORY_V2.md)。

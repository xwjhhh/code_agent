...
默认模型为 `BAAI/bge-m3`。请求使用文档规定的 `model`、`input` 和 `encoding_format: float` 字段。批量文本会在一次请求中发送，返回向量按 `index` 排序。Embedding Key 优先读取 `SILICONFLOW_EMBEDDING_API_KEY`，缺失时回退到 `OPENAI_API_KEY`。
